...
** ID: 933-327-510
** Oregon State Univerity, Fall 2020
** CS 344 - Operating Systems I
** Assignment 6 - Multi-threaded Map Reduce in Rust
** Version Date: 11/30/2020 11:45AM (PDT)
*/

use std::env; // to get arugments passed to the program
use std::thread; // to use threads
...
fn main() {
    ...
    // Change the following code to create 2 threads that run concurrently and each of which uses map_data() function to process one of the two partitions
		...
		let one = xs[0].clone(); //clone first vector to one
		let two = xs[1].clone(); //clone second vector to two
		//spawn threads and add JoinHandles to threads_map
		let threads_map = vec![thread::spawn(move || map_data(&one)), thread::spawn(move || map_data(&two))];
		//join threads and add join values to intermediate_sums
		for t in threads_map{
			let val = t.join().unwrap();
			intermediate_sums.push(val);
		}
    ...
    // 6. Prints the final sum computed by reduce_data
		...
		let partitions : Vec<Vec<usize>> = partition_data(num_partitions,&v); //1
		print_partition_info(&partitions); //2
		let mut threads = vec![];
		for i in 0..num_partitions{ //3
			//clone current partition to be sent to the thread
			let current : Vec<usize> = partitions[i].clone();
			//spawn thread and send current to map_data
			let threaded = thread::spawn(move || map_data(&current));
			//add thread JoinHandle to vector
			threads.push(threaded);
		}
		let mut intermediate_sums_2 : Vec<usize> = Vec::new(); 
		//loop for all threads in threads vector
		for thr in threads { //4
			//wait and join threads
			let val = thr.join().unwrap();
			//add received value to intermediate_sums_2
			intermediate_sums_2.push(val);
		}
		//print intermediate_sums
		println!("Intermediate sums = {:?}", intermediate_sums_2); //5
		//process intermediate_sums_2
		let sum_2 = reduce_data(&intermediate_sums_2); //5
		//print final sum
		println!("Sum = {}", sum_2); //6
}
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
		let mut xs: Vec<Vec<usize>> = Vec::new();
		//check if the length of the vector is divisible by the number of partitions
		
		//when it has a remainder:
		if v.len() % num_partitions != 0 { 
			//number of extra values in vector
			let mut extra = v.len() % num_partitions;
			//length of each partition without the extra values
			let len_par = (v.len() - extra)/num_partitions; //length of partition (base)
			//number of items processed by loop
			let mut num_added = 0;
			for _i in 0..num_partitions{
				//if there are still extra values
				if extra > 0{
					//create a vector and add len_par + 1 values in it
					let mut partition: Vec<usize> = Vec::new();
					for j in num_added..(len_par + 1 + num_added){
						partition.push(v[j]);
					}
					num_added += len_par+1;
					extra = extra - 1;
					//add the vector the the return vector (partitions)
					xs.push(partition);
				} else {
					//create a vector and add len_par values to it
					let mut partition: Vec<usize> = Vec::new();
					for j in num_added..(len_par + num_added){
						partition.push(v[j]);
					}
					num_added += len_par;
					//add vector to vector of vectors
					xs.push(partition);
				}
			}
		}
		//when it is divisible:
			...
			//number of values already added to the vectors
			let mut num_added = 0;
			//length of the partitions
			let len_par = v.len()/num_partitions; //length of partition (base)
			for _i in 0..num_partitions{
				//create vector and add len_par values to it
				let mut partition: Vec<usize> = Vec::new();
				for j in num_added..(len_par + num_added){
					partition.push(v[j]);
				}
				num_added += len_par;
				//push vector to vector of vectors for output
				xs.push(partition);
			}
		}
    xs
